...
/* User-click entry: arms the manual QSO state (target callsign + parity
 * lock + sticky seed) and evaluates the clicked message. Someone else's
 * mid-QSO is rewritten to CQ <call_de> first so compute_one emits Tx2;
 * messages to us keep their normal one-hop reply. Timing (TX window) is
 * the caller's concern; a missed response is regenerated by sticky retry. */

 ...
 * Stable hardware constant; auto_dnf_on_psd uses it for scan/clear timing. */
#define DNF_WORKER_RATE  7350

static auto_dnf_ctx_t *s_dnf_ctx             = NULL;
static ft8_tuning_t     s_dnf_tuning         = {0};
static uint64_t         s_waterfall_slot_id  = 0;
static bool             s_hooks_registered   = false;

static void dnf_init_hook(void) {
    s_dnf_tuning.scan_start_sec = FT8_AUTO_DNF_SCAN_START_SEC_DEFAULT;
    s_dnf_tuning.scan_end_sec   = FT8_AUTO_DNF_SCAN_END_SEC_DEFAULT;
    s_dnf_tuning.clear_time_sec = FT8_AUTO_DNF_CLEAR_TIME_SEC_DEFAULT;
    s_dnf_tuning.min_delta_db   = FT8_AUTO_DNF_MIN_DELTA_DB_DEFAULT;

    ft8_params_load(&s_dnf_tuning);
    s_dnf_ctx = auto_dnf_create(&s_dnf_tuning);
    auto_dnf_snapshot_entry(s_dnf_ctx);
    s_waterfall_slot_id = 0;
}

static void dnf_cleanup_hook(void) {
    if (s_dnf_ctx) {
...
static void dnf_psd_hook(const float *psd, uint16_t nfft,
    ...
    bool have_tx_msg    = ft8_get_tx_msg()->msg[0] != '\0';
    bool is_our_tx_slot = (ft8_is_tx_enabled() && have_tx_msg &&
                           (*ft8_get_tx_time_slot() == info->odd));

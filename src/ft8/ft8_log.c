...
void ft8_log_rx_collect(const slot_info_t *info,
    ...
    uint64_t rf_hz = base_freq_hz + (uint64_t)llroundf(freq_hz);
...
static void log_rx_hook(const char *text, int snr, float freq_hz,
                        float time_sec, ftx_msg_meta_t *meta,
                        const slot_info_t *info) {
    (void)time_sec;
    (void)meta;
    uint64_t base_freq_hz = (uint64_t)subject_get_int(cfg_cur.fg_freq);
    ft8_log_rx_collect(info, base_freq_hz, freq_hz, snr, text);
}

static void log_slot_end_hook(const slot_info_t *info) {
    ft8_log_rx_flush_slot(info);
}

static void log_pre_tx_hook(const slot_info_t *info) {
    uint64_t base_freq_hz = (uint64_t)subject_get_int(cfg_cur.fg_freq);
    uint32_t hz_offset    = (uint32_t)params.ft8_tx_freq.x;
    ft8_log_tx((time_t)info->slot_start, base_freq_hz, hz_offset,
               ft8_get_tx_msg()->msg);
}
...
void ft8_log_register_hooks(void) {
    ...
    ft8_register_init_hook(ft8_log_init);
    ft8_register_cleanup_hook(ft8_log_close);
    ft8_register_rx_msg_hook(log_rx_hook);
    ft8_register_slot_end_hook(log_slot_end_hook);
    ft8_register_pre_tx_hook(log_pre_tx_hook);
}